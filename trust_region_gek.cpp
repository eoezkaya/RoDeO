...
void train_optimal_radius(
	...
	int number_of_outer_iterations = 500;
	...
	max_radius = max(probe_distances);
	...
	while(1){ /* in this loop new radius values are tried */
		...
		radius = random*(max_radius-min_radius);
		...
		for(int outer_iter=0; outer_iter<number_of_outer_iterations; outer_iter++){
			...
			for(int inner_it=0; inner_it< val_set_indices.size(); inner_it++){
				...
				/* distance to the nearest point is too far => use pure Kriging */
				if (distance > radius){

#if 0
					printf("distance = %10.7f is larger than radius = %10.7f\n", distance, radius);
#endif

					fval= calculate_f_tilde(x,
							X_actual,
							beta0,
...
#if 0
					printf("distance = %10.7f is smaller than radius = %10.7f\n", distance, radius);
#endif
					...
					fval= fval + dot((x_not_normalized-sp_not_normalized),grad);
...
#endif
				cv_error+= (fval-fexact)* (fval-fexact);
...
#if 1
		printf("avg_error = %10.7f avg_error_best = %10.7f best_radius = %10.7f\n",avg_cv_error,avg_cv_error_best,best_radius);
#endif
		...
		int max_iter)
{
	...
	int number_of_outer_iterations = 50;
...
int train_TRGEK_response_surface(std::string input_file_name,
	...
	seeding_points.save("TRGEK_seeding_points.dat",raw_ascii);

	std::string points_filename = "TRGEK_seeding_points.dat";
	std::string python_command = "python -W ignore vis_seeding_points.py "+ points_filename+ " "+ std::to_string(X_grad.n_rows);

	FILE* in_TRGEK = popen(python_command.c_str(), "r");
	fprintf(in_TRGEK, "\n");
	...
	if (dim == 2){ /* visualize only linear model response surface */
		...
		for(int i=0;i<resolution;i++){
			...
			for(int j=0;j<resolution;j++){
				...
				rowvec xdiff = sp-xnorm;
				double distance = L2norm(xdiff, dim);

				double factor = exp(-radius*distance);
				...
				if (distance > radius){
					...
					fval= fval + dot((x-sp_not_normalized),grad);
