...
void sortLookupTable(IntLookupTable* table, int indx){

	int n= table->rowsize;
	double *temp = new double[table->columnsize];

	for(int i=1;i<n;++i)
	{
		for(int j=0;j<(n-i);++j)
			if(table->data[indx][j]>table->data[indx][j+1])
			{

				for(int k=0; k<table->columnsize;k++){

					temp[k]=table->data[k][j];
				}

				for(int k=0; k<table->columnsize;k++){

					table->data[k][j] = table->data[k][j+1];
				}

				for(int k=0; k<table->columnsize;k++){

					table->data[k][j+1] = temp[k];
				}
...
/* Vector Lagrange Interpolation implementation with degree N */
/*
 * Given a set of k+1 data points (x_0,y_0),(x_1,y_1),...,(x_k,y_k), where no two x_j are the same
 * the interpolating polynomial is given as:
 * P(x)=sum_{j=0}^{k} y_j*l_j(x)=y_0*l_0(x)+y_1*l_1(x)+...,y_k*l_k(x)
 * the lagrange basis polynomials are given as
 *
 *           (x-x_0)     (x-x_1)           (x-x_{j-1})    (x-x_{j+1})             (x-x_k)
 * l_j(x)=   -------- * --------- * ....* --------------* ------------ * ..... * ----------
 *           (x_j-x_0)   (x_j-x_1)         (x_j-x_{j-1})  (x_j-x_{j+1})           (x_j-x_k)
 *
 */
/* Lagrenge Interpolation implementation with degree N*/

inline void Lagrange_formula(int N, double* x, double* y, double X, double* result ){
	...
	for(j=0; j<N;j++){ /* j=0,1,2,3,..,N-1 */

		prod=1.0;
		for(k=0; k<N;k++){ /* k=0,1,2,3..,N-1 */
...
int interpolateTable1D(IntLookupTable *table,
		double xp,
		int x_index,
		double *result,
		int *comp_index,
	...
	int var1_up,var1_middle,var1_down;
	int i;

	double x[4]; /* coordinates of the interpolation points */
	double f[4]; /* function values at the interpolation points */
	int indx[4]={0,0,0,0}; /* indices of interpolation points */


	/*normalize X */
	xp= (xp-table->xmin[x_index])
																																																						/(table->xmax[x_index]-table->xmin[x_index]);

	/* first we check the last interpolation points */

	if (table->last_interpolation_points[0] != -1){

		//		printf("checking the last interpolation points\n");

		var1_down = table->last_interpolation_points[0];
		var1_up   = table->last_interpolation_points[1];

		if (CheckInterval(xp, table->data[x_index][var1_down], table->data[x_index][var1_up]) == 1){



#if 0
			printf("last interval is still valid\n");
			print_table_element(table,var1_down,0);
			print_table_element(table,var1_up,0);
#endif
			/* 0 1 ... var1_down,var1_up,..., table->rowsize-2,table->rowsize-1  */
			if (var1_down >= 1 && var1_up <= table->rowsize-2)
			{
				indx[0]=var1_down-1;
				indx[1]=var1_down;
				indx[2]=var1_up;
			...
			/* var1_down, var1_up,..., table->rowsize-2,table->rowsize-1  */
			if (var1_down == 0)
			{
			...
			x[0]=table->data[x_index][indx[0]];
			x[1]=table->data[x_index][indx[1]];
			x[2]=table->data[x_index][indx[2]];
			x[3]=table->data[x_index][indx[3]];

			/* for each dependent variable */
			for(i=0; i< number_of_vars_to_interpolate; i++)
			{

				/* get the functional values at four points */
				f[0]=table->data[comp_index[i]][indx[0]];
				f[1]=table->data[comp_index[i]][indx[1]];
				f[2]=table->data[comp_index[i]][indx[2]];
				f[3]=table->data[comp_index[i]][indx[3]];

				/* apply Lagrange formula using 4 interpolation points */

				Lagrange_formula(4, x, f, xp, &result[i] );

				/*correct the result xold = xnew*(xmax-xmin)+xmin*/
				result[i]=result[i]*(table->xmax[comp_index[i]]-table->xmin[comp_index[i]])
																																																											 +table->xmin[comp_index[i]];
	...
	var1_down=0;
	var1_up=table->rowsize-1;
#if 0
	printf("var1_down = %d\n",var1_down);
	printf("var1_up   = %d\n",var1_up);
#endif
	/* Interpolation_var1 is in the interval given by the look-up table : normal treatment */

	if (CheckInterval(xp, 0.0, 1.0) == 1){
		...
		while(!foundflag){
			...
			/* find the index pointing the half of the table */

			var1_middle=(var1_up-var1_down)/2+var1_down;
#ifdef INTERPOLATION_DEBUG
			printf("var1_middle = %d\n",var1_middle);
#endif

			if (CheckInterval(xp, table->data[x_index][var1_down],table->data[x_index][var1_middle]) ==1){
				var1_up=var1_middle; /* variable is in the first half */
			}
			else
				var1_down=var1_middle;  /* variable is in the second half */


			if(var1_up == var1_down+1) foundflag=1;
		...
		x[3]=table->data[x_index][indx[3]];
#if 0
		print_table_element(table, indx[0]);
		print_table_element(table, indx[1]);
		print_table_element(table, indx[2]);
		print_table_element(table, indx[3]);
#endif
		...
		for(i=0; i< number_of_vars_to_interpolate; i++) {
			...
			/*apply Lagrange formula using 4 points */
		...
		/* save the found interval for the next interpolation */

		table->last_interpolation_points[0]=var1_down;
		table->last_interpolation_points[1]=var1_up;



	}

	/* end of normal treatment */


	/* extrapolation : linear interpolation*/

	/* the point is between -infinity and the first data point */

	if ((CheckInterval(xp, -10E14,0) ==1))
	{
#if 0
		printf("input is smaller than 0 => extrapolation\n");
#endif
		double x0=table->data[x_index][0];
		double x1=table->data[x_index][1];
#if 0
		print_table_element(table,0);
		print_table_element(table,1);
#endif
		/* for each variable to be interpolated */

		for(i=0; i< number_of_vars_to_interpolate; i++) {

			double f0=table->data[comp_index[i]][0];
			double f1=table->data[comp_index[i]][1];

			//			printf("f0 = %10.7f\n",f0);
			//			printf("f1 = %10.7f\n",f1);

			double a= (f1-f0)/(x1-x0);
			double b= f1-x1*a;
	...
	/* the point is between the last data point and the plus infinity*/

	if (CheckInterval(xp, 1,10E14) ==1)
	{

#if 0
		printf("input is greater than 1 => extrapolation\n");
#endif
		double x0=table->data[x_index][table->rowsize-1];
		double x1=table->data[x_index][table->rowsize-2];
#if 0
		print_table_element(table,table->rowsize-1);
		print_table_element(table,table->rowsize-2);
#endif
		for(i=0; i< number_of_vars_to_interpolate; i++) {

			/* linear extrapolation using (f0,f2) */

			double f0=table->data[comp_index[i]][table->rowsize-1];
			double f1=table->data[comp_index[i]][table->rowsize-2];
...
void test_2d_table(void){
	...
	int number_of_data_points = 10000;
	int dim_sqr = sqrt(number_of_data_points);

	table2D.Ndim[0]=dim_sqr;
	table2D.Ndim[1]=dim_sqr;
	table2D.Ndim[2]=1;

#if 1
	printf("Table size = %d x %d = %d\n",dim_sqr,dim_sqr,dim_sqr*dim_sqr);
#endif
...
void test_scatter_table(void){
	...
	int ideal_cluster_size = sqrt(tablescatter.rowsize);

	/* cluster the table using cluster_size = 50 */
	KmeansClustering(&tablescatter,ideal_cluster_size,variable_index,2);

	printf("ideal cluster size = %d\n",ideal_cluster_size);



	IntLookupTable tablescatter2;

	tablescatter2.rowsize    = tablescatter.rowsize;
	tablescatter2.columnsize = 5;


	tablescatter2.Ndim[0]=sqrt(tablescatter2.rowsize);
	tablescatter2.Ndim[1]=sqrt(tablescatter2.rowsize);;
	tablescatter2.Ndim[2]=1;


	tablescatter2.data = new double*[tablescatter2.columnsize];

	for(int i=0; i<tablescatter2.columnsize; i++ ){

		tablescatter2.data[i] = new double[tablescatter2.rowsize];

	}


	xs = tablescatter.xmin[2];
	xe = tablescatter.xmax[2];
	ys = tablescatter.xmin[3];
	ye = tablescatter.xmax[3];

	dx = (xe-xs)/(tablescatter2.Ndim[0]-1);
	dy = (ye-ys)/(tablescatter2.Ndim[1]-1);
	...
	int x_index[2] = {2,3};
	int comp_index[3]={0,1,4};

	for(int i=0; i<tablescatter2.Ndim[1]; i++ ){

		x=xs;
		for(int j=0; j<tablescatter2.Ndim[0]; j++){
			...
			tablescatter2.data[0][count]=x;
			tablescatter2.data[1][count]=y;
			tablescatter2.data[2][count]=result[0];
			tablescatter2.data[3][count]=result[1];
			tablescatter2.data[4][count]=result[2];
			count++;
			x+=dx;
		}
	...
	tablescatter2.xmin = new double[tablescatter2.columnsize];
	tablescatter2.xmax = new double[tablescatter2.columnsize];

	for(int i=0; i<tablescatter2.columnsize; i++) { /* for each column of the table */

		tablescatter2.xmin[i]= LARGE;
		tablescatter2.xmax[i]=-LARGE;

		for(int j=0;j<tablescatter2.rowsize;j++){

			if (tablescatter2.data[i][j] < tablescatter2.xmin[i]) tablescatter2.xmin[i]=tablescatter2.data[i][j];
			if (tablescatter2.data[i][j] > tablescatter2.xmax[i]) tablescatter2.xmax[i]=tablescatter2.data[i][j];
		}
	...
	for(int i=0; i<tablescatter2.columnsize; i++) {
		...
		for(int j=0; j<tablescatter2.rowsize; j++){
			...
			tablescatter2.data[i][j]= (tablescatter2.data[i][j]-tablescatter2.xmin[i])/(tablescatter2.xmax[i]-tablescatter2.xmin[i]);
	...
	for(int i=0; i<4; i++) {
		...
		tablescatter2.last_interpolation_points[i]=-1;
	}

#if 1
	printTable(&tablescatter2, 0);
#endif
...
void test_scatter_table2(void){
	...
	int number_of_interpolations = 10000;

	FILE *inp = fopen("gastable.dat","r");

	IntLookupTable tablescatter;

	tablescatter.rowsize    = 50000;
	tablescatter.columnsize = 8;
	...
	for(int i=0; i<tablescatter.rowsize; i++){
		...
		fscanf(inp,"%lf %lf %lf %lf %lf %lf %lf %lf ",
				&tablescatter.data[0][i],
				&tablescatter.data[1][i],
				&tablescatter.data[2][i],
				&tablescatter.data[3][i],
				&tablescatter.data[4][i],
				&tablescatter.data[5][i],
				&tablescatter.data[6][i],
				&tablescatter.data[7][i]);
	...
	for(int i=0; i<tablescatter.columnsize; i++) {
		...
		printf("min[%d] = %10.7f max[%d] = %10.7f\n",i,tablescatter.xmin[i],i,tablescatter.xmax[i]);
	}
	...
	for(int i=0; i<4; i++) {
		...
		tablescatter.last_interpolation_points[i]=-1;
	}
	...
	/* cluster the table using cluster_size = 50 */
	KmeansClustering(&tablescatter,ideal_cluster_size,variable_index,1);
	...
	int x_index= 5;
	int comp_index[2]= {0,4};
	double result[3];
	// Record start time
	auto start = std::chrono::high_resolution_clock::now();


	for(int i=0; i<number_of_interpolations; i++){


		//    	if(i%1000 == 0) printf("i = %d\n",i);

		double x= RandomDouble(tablescatter.xmin[7],tablescatter.xmax[7]);
	...
	sortLookupTable(&tablescatter,5);
	...
	for(int i=0; i<number_of_interpolations; i++){
		...
		interpolateTable1D(&tablescatter,
				x,
				x_index,
				result,
				comp_index,
