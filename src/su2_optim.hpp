...
public:
	...
	OptimizationData(int input_size){
		...
		number_of_active_design_variables = input_size;
		variable_activity.ones(input_size);
	...
	~OptimizationData(){
		...
		printf("Deleting %s\n", name.c_str());
#endif
		lower_bound_dv.reset();
		upper_bound_dv.reset();
		variable_activity.reset();
