...
void testAllKernelRegression(void){
	...
	cout << "Running unit test for the kernel regression model...\n";
...
void testcalculateGaussianKernelAdjoint(void){
	...
	int indx1 = generateRandomInt(0,10);
	int indx2 = generateRandomInt(10,20);
	rowvec xi = testModel.X.row(indx1);
	rowvec xj = testModel.X.row(indx2);
...
void testGradientEnhancedKernelRegression(void){
	...
	LinearTestFunc.setVisualizationOn();
	LinearTestFunc.setBoxConstraints(-5.0,5.0);
	LinearTestFunc.print();
	double testError = LinearTestFunc.testSurrogateModel(GRADIENT_ENHANCED_KERNEL_REGRESSION,100);
	bool passTest = checkValue(testError, 0.0, 10E-6);
	abortIfFalse(passTest,__FILE__, __LINE__);
}
