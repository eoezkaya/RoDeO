...
mat generate2DEggholderTestDataForSGEKModel(unsigned int N){

	TestFunction testFunctionEggholder("Eggholder",2);

	testFunctionEggholder.setFunctionPointer(EggholderAdj);

	testFunctionEggholder.setBoxConstraints(0,200.0);
	mat samples = testFunctionEggholder.generateRandomSamplesWithGradients(100);
...
class SGEKModelTest : public ::testing::Test {
protected:
	void SetUp() override {


		unsigned int N = 100;
		samples = generate2DEggholderTestDataForSGEKModel(100);
		samples.print();
